...
/mob/living/proc/adjust_chi_pool(amount, updating_health = TRUE, on_spawn, chi_type)
	...
	switch(chi_type)
		if("Yin")
			yin_chi = clamp(yin_chi+amount, 0, max_yin_chi)
		if("Yang")
			yang_chi = clamp(yang_chi+amount, 0, max_yang_chi)
    	if("Demon")
			demon_chi = clamp(demon_chi+amount, 0, max_demon_chi)
    	if("Tainted Yin")
			tainted_yin_chi = clamp(tainted_yin_chi+amount, 0, max_tainted_yin_chi)
		if("Tainted Yang")
			tainted_yang_chi = clamp(tainted_yang_chi+amount, 0, max_tainted_yang_chi)
		if("Golden")
